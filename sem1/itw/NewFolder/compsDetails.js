...
var vm = function () {
    ...
    var athletecounter=0
    //--- Page Events
    self.activate = function (sportId) {
        ...
        ajaxHelper(composedUri, 'GET').done(function (data) {
            ...
            hideLoading();
            self.details(data);
            athletecounter = data.Athletes.length
            self.athletes(data.Athletes)
            console.log(self.athletes(), athletecounter)
            self.linkableName = ko.observable('')
            self.WikipediaLink(getWikipediaArticle(data.Name, sportId));
        });
    };
    ...
    function getWikipediaArticle(keyword, keyword2) {
        if (!keyword) {return}
        var searchword = keyword + ' ' + keyword2 + " Olympics"
