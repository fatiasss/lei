...
var vm = function () {
    ...
    self.prevCoords = { lat: 37.645740, lng: 21.625454 }
    ...
    self.nextmark = function () {
        ...
        while (locationnext && (locationnext.Lat == null || locationnext.Lon == null)) {
            currentLocationIndex++;
            locationnext = locations[currentLocationIndex + 1];// Skip to the next location
        }
        if (!location || location.Lat == null || location.Lon == null) {
            console.error("Invalid location.");
            if (currentLocationIndex == 0) {
                if (locationprev && locationprev.Lat != null && locationprev.Lon != null) { //tbm tens que aumentar isto memo que o coiso seja nulo ne
                    self.prevCoords = { lat: locationprev.Lat, lng: locationprev.Lon };
                } else {
                    self.prevCoords = { lat: location.Lat, lng: location.Lon };
                }
            }
            else {
                if (locationprev && locationprev.Lat != null && locationprev.Lon != null) {
                    self.prevCoords = { lat: locationprev.Lat, lng: locationprev.Lon };
                } else {
                    self.prevCoords = { lat: location.Lat, lng: location.Lon };
                }
...
function addPins(locations, self) {
    ...
    return locations.map(location => {
        ...
        labelDiv.style.zIndex = 100000000; 
...
function emitArc({ lat: endLat, lng: endLng }) {
    ...
    let prevCoords = self.prevCoords || { lat: 37.645740, lng: 21.625454 };
