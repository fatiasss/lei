...
var vm = function () {
    ...
    self.displayName = 'Your Favourite Technicians List';
    ...
    self.toggleFavourite = function (athlete) {
        if (self.isFavourite(athlete.Id)) {
            self.favourite_techs.remove(athlete.Id);
            localStorage.setItem('favourite_techs', JSON.stringify(self.favourite_techs()));
        } else {
            self.favourite_techs.push(athlete.Id);
            localStorage.setItem('favourite_techs', JSON.stringify(self.favourite_techs()));
        }
    }
    self.isFavourite = function (athleteId) {
        return (self.favourite_techs().includes(athleteId))
    }
    ...
    self.activate = async function () { //had to rewrite the whole thing so it could have the delay and be asynchronous
        ...
        self.loadFavourites();
        for (const techId of self.favourite_techs()) {
            const composedUri = `${self.baseUri()}/${techId}`;
            await ajaxHelper(composedUri, 'GET').done(function (data) {
                console.log(data);
                hideLoading();
                self.techs.push(data);
            });
