...
PersonSet *PersonSetCreate() {
  ...
  PersonSet* pSet = malloc(sizeof(struct _PersonSet_ ));
  pSet->persons= ListCreate(cmpP);
  return pSet;
}
...
PersonSet *PersonSetIntersection(const PersonSet *ps1, const PersonSet *ps2) {
  PersonSet *ps = PersonSetCreate();
  for(ListMoveToHead(ps1->persons); ListCurrentIsInside(ps1->persons); ListMoveToNext(ps1->persons)){
    Person* currentItem = ListGetCurrentItem(ps1->persons);
    if(ListSearch(ps2->persons,currentItem)==0){
      ListInsert(ps->persons, ListGetCurrentItem(ps1->persons));
    }
  }
