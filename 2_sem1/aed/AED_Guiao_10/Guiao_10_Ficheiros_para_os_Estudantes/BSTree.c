...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  int lh=_treeGetHeightREC(root->left);
  int rh=_treeGetHeightREC(root->right);
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  struct _BSTreeNode* smallest_leaf = root->left;

  while (0==0){
    if(smallest_leaf->left==NULL) break;
...
static void* _treeGetMax(const struct _BSTreeNode* root) {
  ...
  return root->right!=NULL? _treeGetMax(root->right) : root->item;
...
void* BSTreeSearch(const BSTree* header, const void* item) {
  ...
  while (current != NULL) {
    ...
    int comparison = header->compare(item, current->item);
