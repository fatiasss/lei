...
Graph* GraphCopy(const Graph* g) {
  assert(g != NULL);
  Graph* g2 = GraphCreate(g->numVertices, g->isDigraph, 0);
  g2->isWeighted=g->isWeighted;
  g2->isComplete=g->isComplete;
...
int GraphCheckInvariants(const Graph* g) {
  ...
  return g->numEdges>=0 && g->numEdges>=0 && (g->isComplete || !g->isComplete) && (g->isDigraph || !g->isDigraph) && g->verticesList
