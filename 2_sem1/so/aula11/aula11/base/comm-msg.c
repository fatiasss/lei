...
static long clients[2];
static int nclients;

static void add_client(long pid){
    if(nclients>=2) return;

    clients[nclients++] = pid;
}
...
void servReceive(MESSAGE *msg)
{
    ...
    *msg = container.msg;
    ...
    add_client(container.client);
}

void servSend(MESSAGE *msg)
{
    /* send a message */
    long src = container.client;
    long dest = clients[0]==container.client ? clients[1] : clients[0];
    if (!dest) dest = src;
    container.msg = *msg;
    container.client=dest;
